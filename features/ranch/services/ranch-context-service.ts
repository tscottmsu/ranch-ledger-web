...
export async function getCurrentRanchContext(): Promise<CurrentRanchContext | null> {
  ...
  const { data: ranch, error: ranchError } = await supabase.from("ranches").select("name").eq("id", membership.ranch_id).single();
  if (ranchError) throw ranchError;
  return { ranchId: membership.ranch_id, ranchName: ranch.name, role: membership.role as CurrentRanchContext["role"] };
}
