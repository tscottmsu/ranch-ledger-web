  ...
  return <details className="group rounded-lg border border-orange-200 bg-white shadow-sm open:shadow-md">
    ...
    <form action={createRideAction} className="grid gap-3 border-t border-orange-100 p-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="grid gap-1 text-sm font-medium text-stone-700">Ride type
          <select name="activityType" required className="h-9 rounded-lg border bg-white px-3 text-sm">
      ...
      <Button type="submit" disabled={!activityTypes.length} className="bg-orange-700 text-white hover:bg-orange-800">Create ride</Button>
      {!activityTypes.length && <p className="text-sm text-red-700">Create an active ride activity type before building rides.</p>}
    </form>
  </details>;
}
