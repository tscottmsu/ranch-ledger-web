...
export function DashboardShell({ ranchName, email, children }: { ranchName?: string; email?: string; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-muted/30 lg:pl-64">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r bg-sidebar p-5 text-sidebar-foreground lg:flex lg:flex-col">
      <Brand href="/dashboard" className="text-sidebar-foreground" />
      <div className="mt-7 rounded-xl border border-sidebar-border bg-white/5 p-3"><p className="truncate text-sm font-medium">{ranchName ?? "Ranch setup"}</p><p className="mt-1 truncate text-xs text-sidebar-foreground/55">{email}</p></div>
      <Navigation pathname={pathname} />
      <form action={logoutAction} className="mt-auto"><Button type="submit" variant="ghost" className="w-full justify-start text-sidebar-foreground/70 hover:bg-sidebar-accent/20 hover:text-sidebar-foreground"><LogOut />Sign out</Button></form>
    </aside>
